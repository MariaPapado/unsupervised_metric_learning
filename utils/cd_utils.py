...
def image_loss(img1, img2, change, no_change, lossf_s, lossf_d):
    ...
    no_change_s = no_change.unsqueeze(1) #.unsqueeze(0)
    change_s    = change.unsqueeze(1) #.unsqueeze(0)
    #print('shapes', no_change_s.shape, change_s.shape)  #[1, 1, 256, 256]
    ...
    #Loss similarity
    #print('aaaaaaa', no_change_s.shape, img1.shape)
    loss_s  = lossf_s(no_change_s*img1, no_change_s*img2)
#    loss_s  = MahalanobisDis(no_change_s*img1, no_change_s*img2)
    
    #Loss dissimilarity
    loss_d  = -lossf_d(change_s*img1, change_s*img2)
#    loss_d  = -MahalanobisDis(change_s*img1, change_s*img2)
...
def feature_loss(img1_feat, img2_feat, change, no_change, lossf_s, lossf_d, N_vgg):
    loss_s = 0.0
    loss_d = 0.0
    #print('aaaaaaa', change.shape, no_change.shape, img1_feat[0].shape, img2_feat[0].shape)
    for i in range(N_vgg):
        img1 = img1_feat[i]
        img2 = img2_feat[i]
        #print('aaaaaaaaaa', i, img1.shape, img2.shape)
        ...
        if i==0:
            ...
            no_change_s = nn.functional.interpolate(no_change.unsqueeze(1), scale_factor=1/2**i, mode='bicubic')
            change_s    = nn.functional.interpolate(change.unsqueeze(1), scale_factor=1/2**i, mode='bicubic')
...
# Feature Loss
def contrastive_loss(img1_feat, img2_feat, distance_measure3, N_vgg):
    loss     =  0.0
    for i in range(N_vgg):
        img1 = img1_feat[i]
        img2 = img2_feat[i]
        ...
        #Loss dissimilarity
        loss = loss + distance_measure3(img1, img2)
    loss = loss/N_vgg
    return loss
